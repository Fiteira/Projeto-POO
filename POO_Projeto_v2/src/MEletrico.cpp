...
MEletrico::MEletrico(Fabrica *F, int _id, string _marca, int _posx, int _posy, float _consumo) : Motor(F, _id, _marca, _posx, _posy, _consumo)
{
    //ctor
    cout << "Passei no metodo <" << __FUNCTION__ << "> na classe MEletrico" << endl;
    Set_TEMP_AV_MELETRICO();
}
...
bool MEletrico::RUN()
{
    ...
    if (Get_ESTADO() == ESTADO_MOTOR::ESTADO_START || Get_ESTADO() == ESTADO_MOTOR::ESTADO_RUN)
    {
        ...
        {
            float Verde_Min = Get_Ptr_Fabrica()->Get_DEF_MELETRICO(0);
            float Verde_Max = Get_Ptr_Fabrica()->Get_DEF_MELETRICO(1);
            ...
            if (TEMPERATURA >= Verde_Min && TEMPERATURA <= Verde_Max)
                TEMPERATURA_VERDE();
            else if (TEMPERATURA >= TEMPERATURA_AVISO && TEMPERATURA <= Get_Ptr_Fabrica()->Get_DEF_MELETRICO(3))
                TEMPERATURA_AMARELO();
            else if (TEMPERATURA >= TEMPERATURA_PARAGEM && TEMPERATURA <= Get_Ptr_Fabrica()->Get_DEF_MELETRICO(5))
            {
